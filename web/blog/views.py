...
def blog(request):
    ...
    # Si la petición es AJAX (XHR), devolver JSON
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        for p in productos:
            data.append({
                ...
                'categoria_nombre': p.categorias.first().nombre if p.categorias.exists() else None,
                'detalle_url': reverse('producto_detalle', args=[p.id]),
                'agregar_carro_url': reverse('carro:agregar', args=[p.id])
            })
